...
import com.manulife.studentportal.academic.ExamMapper;
...
import com.manulife.studentportal.academic.ExamService;
...
public class ExamServiceImpl implements ExamService {
    ...
    public ExamResponse create(CreateExamRequest request) {
        ...
        if (securityService.isTeacher()) {
            ...
            boolean assignedToClass = teacher.getClassIds().contains(request.getClassId());
            boolean assignedToSubject = teacher.getSubjectIds().contains(request.getSubjectId());
    ...
    public Page<ExamResponse> getAll(Pageable pageable) {
        ...
        if (securityService.isAdmin()) {
            ...
            List<Long> assignedClassIds = new java.util.ArrayList<>(teacher.getClassIds());
            ...
            Page<Exam> exams = examRepository.findBySchoolClassId(studentInfo.classId(), pageable);
            return exams.map(examMapper::toResponse);
        }
    ...
    public ExamResponse update(Long id, UpdateExamRequest request) {
        ...
        if (securityService.isTeacher()) {
            ...
            boolean assignedToClass = teacher.getClassIds().contains(examClassId);
            boolean assignedToSubject = teacher.getSubjectIds().contains(examSubjectId);
